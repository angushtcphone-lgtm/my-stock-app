...
active_tickers = [t.strip().upper() for t in ticker_input.split(",") if t.strip()]
...
for ticker in active_tickers:
    with st.sidebar.expander(f"⚙️ {ticker} 戰略配置規劃", expanded=False):
        # 讓用戶直覺選擇是否有持倉
        has_pos = st.checkbox("我已持有此部位", value=(ticker in ['RKLB', 'AVGO', 'WMT']))
        
        if has_pos:
            # 如果有持倉，預設帶入你先前的真實成本
            default_cost = 71.36 if ticker == 'RKLB' else 0.0
            cost = st.number_input(f"{ticker} 持倉成本 (美元/台幣)", value=default_cost, step=0.1)
        else:
            cost = None
            
        target = st.number_input(f"{ticker} 戰略目標價", value=110.0 if ticker == 'RKLB' else 0.0, step=1.0)
        
        # 決定是買點還是賣點類型
        action_type = st.selectbox(f"{ticker} 目標類型", ["BUY_TARGET", "SELL_TARGET", "HOLD"], 
                                    index=1 if ticker == 'RKLB' else 0)
        
        # 直接在網頁上輸入戰略對齊文字！
        default_desc = "限價單已準備，衝高至 $110 獲利出清" if ticker == 'RKLB' else "下殺至目標價附近執行金字塔建倉"
        action_desc = st.text_input(f"{ticker} 戰略部署規劃", value=default_desc)
