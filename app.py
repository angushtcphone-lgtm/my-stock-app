...
if market_choice == "🇺🇸 美股核心指揮部":
    ...
    # 🔥 【優化點】：台股預設標的直接拿掉 .TW，展示全新自動補齊功能
    default_tickers = "2330, 2454, 2317"
    currency_sign = "NT$"

st.caption(f"即時數據源：Yahoo Finance | 2026 版本：智慧公司名稱拉取 + 雙戰略分類分頁系統")
...
# 🔥 【核心防呆修復點 2】：台股免打 .TW！如果是純數字，後台自動補上字尾
active_tickers = []
...
# ==============================================================================
# 📦 【核心修復點 3】：全新戰略級分頁，正式劃分「持倉」與「觀察」兩大戰場
# ==============================================================================
ui_tabs = st.tabs(["🛡️ 核心主力持倉面板", "⏳ 戰略觀察伏擊名單"])

with ui_tabs[0]:
    if holding_matrix:
        st.dataframe(pd.DataFrame(holding_matrix), use_container_width=True)
    else:
        st.info("💡 目前該市場尚無持倉部位。可在左側控制艙勾選『我已持有此部位』來點亮資產。")

with ui_tabs[1]:
    if watching_matrix:
        st.dataframe(pd.DataFrame(watching_matrix), use_container_width=True)
    else:
        st.info("💡 所有自選股皆已納入持倉。可在左側輸入新代碼建立全新觀察哨！")
