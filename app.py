...
st.title("🎯 專屬美股/台股戰略儀表板：全功能完全體操盤中樞")
st.caption("即時數據源：Yahoo Finance | 2026 終極完全體：動態標籤 + 側邊欄滾動修正 + 即時損益計算")

# 🔥 【核心修復點 1】：強行注入 CSS 樣式表，強制左側控制艙在溢出時自動出現獨立滾動條，100% 解決無法滑到底的 Bug
st.markdown(
    """
    <style>
...
for ticker in active_tickers:
    ...
    try:
        ...
        # 🔥 【核心修復點 3】：自動計算即時持倉損益百分比
        if cfg['cost'] and cfg['cost'] > 0:
            cost_display = f"${cfg['cost']:.2f}"
...
with tab3:
    ...
    * **【低於 30% ➔ 👑 散戶踩踏區】** 籌碼極度分散，風吹草動大家就會互相踩踏，波動劇烈。
    """)
